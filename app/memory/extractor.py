...
_SYSTEM_PROMPT = (
    ...
    "(e.g. 'remember that when I say X you should Y', 'keep this in "
    "mind', 'don't forget that...'). Treat this as a strong signal "
    "regardless of how trivial it sounds. Even a test memory like "
    "'when I say pineapple, reply with a pineapple recipe' should be "
    "saved \u2014 the user is explicitly teaching the assistant.\n"
    "\n"
    "Also save when (without an explicit request) the conversation "
    "reveals:\n"
    "- phrase mappings the user implicitly taught the assistant (e.g. "
    "their words for a custom form);\n"
    ...
    "in the input.\n"
    "\n"
    "When the user asks you to remember something, default to creating a "
    "memory with a stable `key` so future conversations can update it "
    "in place instead of duplicating. For phrase->action triggers use a "
    "key like `trigger:pineapple` or `phrase:remember-x`.\n"
    "\n"
    "When in doubt about non-explicit memories, save nothing. But for "
    "explicit 'remember this' requests, always save."
)
